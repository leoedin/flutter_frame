...
void main() {
  ...
  String? optionalEnvVariable(String name) {
    var data = env[name];
    if (data != null) {
      log.info("OptionaEnv variable $name is $data");
    }
    return data;
  }

  var albumUrls = [requiredEnvVariable("GALLERY_URL")];
  for (int i = 2; i < 6; i++) {
    final envVar = optionalEnvVariable("GALLERY_URL_$i");
    if (envVar != null) {
      albumUrls.add(envVar);
    }
  }
  ...
  runApp(PhotoGalleryApp(albumUrls, delayS, shuffle));
}
...
class PhotoGalleryApp extends StatelessWidget {
  ...
  PhotoGalleryApp(this.albumUrls, this.delayS, this.shuffle, {Key? key})
      : imageBackend = GPhotoImageBackend(albumUrls),
        super(key: key);
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
        title: "Photo Gallery",
        home: Center(child: PhotoGalleryWidget(imageBackend, delayS, shuffle)));
  }
}
...
class PhotoGalleryWidget extends StatefulWidget {
  ...
  const PhotoGalleryWidget(this.imageBackend, this.delayS, this.shuffle,
      {Key? key})
      : super(key: key);
...
class _PhotoGalleryWidgetState extends State<PhotoGalleryWidget> {
  ...
  Future<List<Image>> retryGetImages() {
    ...
    return widget.imageBackend.getImages(photoHeight).then((value) {
